...
void LoadMusic (FSOUND_STREAM *musique, int channel, int *nb_mus, char *titre, char *auteur, char *album)
{
    ...
    FSOUND_Stream_Close(musique) ; //On libère la musique.

    if (*nb_mus <= -1) *nb_mus = 10 ;
    if (*nb_mus >= 11) *nb_mus = 0 ;

    sprintf(nom_fichier, "musiques/%d.mp3", *nb_mus) ; //On écrit le nom du fichier à charger...
    musique = FSOUND_Stream_Open(nom_fichier, 0, 0, 0) ; //On lis la nouvelle.
    if (musique == NULL) //Vérification.
    {
        fprintf(stderr, "Impossible de lire : %s.", nom_fichier) ;
        exit(EXIT_FAILURE) ;
    }
    FSOUND_Stream_Play(channel, musique) ;
    FSOUND_Stream_GetNumTagFields(musique, &nb_tag) ; //Avec ça, on obtient le nombre de tags.
    printf("Tags (%d) :\n", nb_tag) ;
    for (i=0 ; i<nb_tag ; i++)
    {
    ...
    SDL_WM_SetCaption(nom_fenetre, NULL) ; //On change l'entête de la fenêtre.
